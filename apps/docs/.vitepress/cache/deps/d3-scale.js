...
var rgb_default = (function rgbGamma(y) {
  ...
  function rgb2(start, end) {
    var r = color2((start = rgb(start)).r, (end = rgb(end)).r),
...
function hsl2(hue2) {
  return function (start, end) {
    var h = hue2((start = hsl(start)).h, (end = hsl(end)).h),
...
function hcl2(hue2) {
  return function (start, end) {
    var h = hue2((start = hcl(start)).h, (end = hcl(end)).h),
...
function cubehelix2(hue2) {
  return (function cubehelixGamma(y) {
    ...
    function cubehelix3(start, end) {
      var h = hue2((start = cubehelix(start)).h, (end = cubehelix(end)).h),
        s = nogamma(start.s, end.s),
        l = nogamma(start.l, end.l),
        opacity = nogamma(start.opacity, end.opacity);
      return function (t) {
...
function transformer() {
  ...
  function scale(x) {
    return x == null || isNaN((x = +x))
      ? unknown
      : (output || (output = piecewise2(domain.map(transform), range2, interpolate)))(
          transform(clamp(x)),
        );
  }
  scale.invert = function (y) {
    return clamp(
      untransform(
        (input || (input = piecewise2(range2, domain.map(transform), number_default)))(y),
      ),
    );
  };
  scale.domain = function (_) {
...
function powish(transform) {
  var scale = transform(identity2, identity2),
    exponent = 1;
  function rescale() {
    return exponent === 1
      ? transform(identity2, identity2)
      : exponent === 0.5
...
function radial() {
  ...
  scale.range = function (_) {
    return arguments.length
      ? (squared.range((range2 = Array.from(_, number)).map(square)), scale)
      : range2.slice();
  };
  scale.rangeRound = function (_) {
  ...
  function scale(x) {
    return x == null || isNaN((x = +x))
      ...
      : interpolator(
          k10 === 0
            ? 0.5
            : ((x = (transform(x) - t03) * k10), clamp ? Math.max(0, Math.min(1, x)) : x),
        );
  }
  scale.domain = function (_) {
    return arguments.length
...
function transformer3() {
  ...
  function scale(x) {
    return isNaN((x = +x))
      ? unknown
      : ((x = 0.5 + ((x = +transform(x)) - t13) * (s * x < s * t13 ? k10 : k21)),
