# @vitecharts/react

## 1.0.0

### Major Changes

- ViteCharts 1.0 — first stable release. Animated, framework-agnostic charts with
  the feel of ApexCharts: the cartesian/radial/statistical/flow chart packs, the
  animation and interaction layers, CSS-variable theming, tree-shakeable
  registration, and React / Vue / Web Component bindings plus an ApexCharts compat
  shim. APIs are now considered stable under semver.

### Minor Changes

- 04fcce2: Framework adapters over the shared core:
  - `@vitecharts/wc` — `<vitecharts-chart>` web component (zero deps, any framework).
  - `@vitecharts/react` — `<Chart />` with a ref to the imperative instance.
  - `@vitecharts/vue` — `<ViteChart :options>` with reactive deep updates.

  Each creates one core `Chart`, applies option changes, and tears down on unmount.

### Patch Changes

- Updated dependencies [e6cd903]
- Updated dependencies [2dbbdc8]
- Updated dependencies [e932cd1]
- Updated dependencies [693f099]
- Updated dependencies [b64de0e]
- Updated dependencies [05adc49]
- Updated dependencies [b40b1eb]
- Updated dependencies [ffd8565]
- Updated dependencies [8413459]
- Updated dependencies [9a73461]
- Updated dependencies [58f8d72]
- Updated dependencies [70a7582]
- Updated dependencies [18652d5]
- Updated dependencies [b828afb]
- Updated dependencies [83e6edc]
- Updated dependencies [5e27867]
- Updated dependencies
- Updated dependencies [02fbf47]
- Updated dependencies [751bbd4]
  - @vitecharts/core@1.0.0
